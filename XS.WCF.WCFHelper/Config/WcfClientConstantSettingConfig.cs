...
using XS.WCF.WCFHelper.LoadBalance;
using System.Reflection;
namespace XS.WCF.WCFHelper.Config
{
    internal class WcfClientConstantSettingConfig
    {
        private IDictionary<string, WcfClientBinding> mlist;

        /// <summary>
        /// 客户端常量设置配值类
        /// </summary>
        /// <param name="doc"></param>
        public WcfClientConstantSettingConfig(XmlDocument doc)
        {
            mlist = new Dictionary<string, WcfClientBinding>();
            try
            {
                foreach (XmlElement elem in XmlHelper.Children(doc.DocumentElement, "Servers"))
                {
                    foreach (XmlElement xe in XmlHelper.Children(elem, "Server"))
                    {
                        ...
                        WcfClientBinding bing = new WcfClientBinding()
                        {
        ...
        /// 读取列表配置
        /// </summary>
        public IDictionary<string, WcfClientBinding> WcfClientConstantList
        {
            get
            {
                return mlist;
    ...
    /// 客户端常量配值实体
    /// </summary>
    internal class WcfClientBinding
    {
