...
using XS.WCF.WCFHelper.SerializeBehavior;
...
namespace XS.WCF.WCFHelper.Config
{
    internal class WcfCacheData
    {
        ...
        private static volatile IDictionary<string, IDictionary<string, WcfClient>> clientsServiceList = null;
        private static volatile IDictionary<string, WcfClientBinding> wcfClientConstantList = null;
        ...
        public static void Init()
        {
            ...
            if (!isInitialization)
            {
                lock (lockhelper)
                {
                    if (!isInitialization)
                    {
                        WcfClientSetting config = new WcfClientSetting();
                        //根据契约名获取部署服务器
                        clientsServiceReverseList = config.WcfClient.ClientsServiceReverseList;
                        //根据服务器名获取客户端契约哈希表
                        clientsServiceList = config.WcfClient.ClientsServiceList;
                        //根据服务器名获取客户端常量配置
                        wcfClientConstantList = config.WcfClientConstant.WcfClientConstantList;
                        //标记客户端配置初始化完成
        ...
        /// <param name="server_name"></param>
        /// <param name="wcfcbing"></param>
        public static WcfClientBinding GetWcfClientConst(string server_name)
        {
            if (!wcfClientConstantList.ContainsKey(server_name))
            {
                throw new Exception("没有注册此契约对应的服务器配置,请检查常量配置文件,服务器配置名称:" + server_name);
            }
            //根据服务器名获取常量配置
            return wcfClientConstantList[server_name];
        }
        ...
        /// <param name="client"></param>
        public static WcfClient GetContract(string contract)
        {
        ...
        {
            string server_name = GetServerName(contract);
            WcfClientBinding wcfcbing = GetWcfClientConst(server_name);
        ...
        private static EndpointAddress GetAddress(string contract, string uri)
        {
            string server_name = GetServerName(contract);
            WcfClientBinding wcfcbing = GetWcfClientConst(server_name);
            WcfClient client = GetContract(contract);
        ...
        public static void GetCachedData(string contract, out NetTcpBinding binging, out EndpointAddress address, out WcfClient client, out WcfClientBinding wcfcbing, out string server_name)
        {
            //获取常量
        ...
        public static bool IsLoadBalance(string contract)
        {
            Init();
            string server_name = GetServerName(contract);
            WcfClientBinding bing = GetWcfClientConst(server_name);
            bool flag = bing.LoadBalance.IsUsed;
            //如果启用负载均衡，则启动监听心跳
            if (flag && !wcfClientConstantList[server_name].LoadBalance.BalanceAlgorithm.IsNewInstance)
            {
                LoadBalanceConfig balance = wcfClientConstantList[server_name].LoadBalance;
        ...
        public static ChannelFactory<T> GetFactory<T>(Address uri_address, bool EnableBinaryFormatterBehavior)
        {
            ...
            if (client == null)
            {
                ...
                WcfClient client_contract = GetContract(contract);
                client = new ChannelFactory<T>(binging, address);
                //增加头信息行为
                ClientMessageInspector msgBehavior = new ClientMessageInspector();
                client.Endpoint.Behaviors.Add(msgBehavior);
