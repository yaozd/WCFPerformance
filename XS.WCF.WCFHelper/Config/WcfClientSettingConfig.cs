...
{
    internal class WcfClientSettingConfig
    {
        private IDictionary<string, IDictionary<string, WcfClient>> mlist;
        private IDictionary<string, string> mlist1;
        /// <summary>
        /// 客户端服务读取配值文件
        /// </summary>
        /// <param name="doc"></param>
        public WcfClientSettingConfig(XmlDocument doc)
        {
            mlist = new Dictionary<string, IDictionary<string, WcfClient>>();
            mlist1 = new Dictionary<string, string>();
            IDictionary<string, WcfClient> list2;
            foreach (XmlElement elem in XmlHelper.Children(doc.DocumentElement, "Clients"))
            {
                foreach (XmlElement xe in XmlHelper.Children(elem, "Server"))
                {
                    ...
                    list2 = new Dictionary<string, WcfClient>();
                    foreach (XmlElement xe1 in XmlHelper.Children(xe, "client"))
                    {
                        if (xe1.NodeType != XmlNodeType.Comment)
                        {
                            ...
                            list2.Add(contract, new WcfClient()
                            {
                                Contract = contract,
                                Address = address,
        ...
        /// <summary>
        /// 读取以name为key 其它值为wcfClient为值的列表
        /// </summary>
        public IDictionary<string, IDictionary<string, WcfClient>> ClientsServiceList
        {
            get
            {
                return mlist;
